...
```{r message=FALSE, warning=FALSE}
path_to_data_list <- "./dataList.csv" #only glu and gluaa, from david
...
mydata %>% 
  ...
  facet_wrap(~well,scale="free")+
  theme_cowplot()

# There we can keep only A and E.

mean_blank_od <- mydata %>% 
  filter(strain=="empty") %>% 
  filter(row %in% c("A","E")) %>% 
  mutate(mean_blank_od=mean(od)) %>% 
  mutate(sd_blank_od=sd(od)) %>% 
  filter(!between(od,mean_blank_od-2*sd_blank_od,mean_blank_od+2*sd_blank_od)) %>% 
  mutate(mean_blank_od=mean(od))
